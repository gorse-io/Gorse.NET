...
public class Tests
{
    ...
    public async Task TestUsersAsync()
    {
        ...
        Assert.That(rowAffected.RowAffected, Is.EqualTo(1));
        // Get this user.
        var returnUser = await client.GetUserAsync("1");
        Assert.That(returnUser, Is.EqualTo(user));
        // Delete this user.
        rowAffected = await client.DeleteUserAsync("1");
        Assert.That(rowAffected.RowAffected, Is.EqualTo(1));
        try
        {
            returnUser = await client.GetUserAsync("1");
            Assert.Fail();
        }
        catch (GorseException e)
        {
    ...
    {
        var db = redis.GetDatabase();
        await db.SortedSetAddAsync("offline_recommend/10", new SortedSetEntry[]
        {
            new SortedSetEntry("10", 1),
            new SortedSetEntry("20", 2),
            new SortedSetEntry("30", 3)
